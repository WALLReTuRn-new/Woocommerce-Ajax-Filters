        ...
        var initCartAction = function (self) {
            ...
            self.$wrapper.on('click', '.btn-cart', function (e) {
                ...
                if (
                    ...
                    /*Donald.Minipopup.open({
                     message: 'Sepete eklendi... <a href="/cart" class="btn btn-link btn-sm btn-slide-right btn-infinite">SEPETД°M <i class="d-icon-arrow-right"></i></a>',
            ...
            Donald.Minipopup = (function () {
                ...
                return {
                    init: function () {
                        ...
                        Donald.byClass('woocommerce-page')[0].appendChild(area);
                            ...
                            .on('mousedown', '.select-menu', function (e) {
                                ...
                                if ($selectMenu.is($target.parent())) {
                                    ...
                                    if ($target.parent().hasClass('active')) { // add select-item, and show
                                        ...
                                        $('.select-items').show();
                                        $('<a href="#" class="select-item filters">' + $target.text() + '<i class="fas fa-times"></i></a>')
                                                .insertBefore('.select-items .filter-clean')
                                                .hide().fadeIn()
                                                .data('link', $target.parent())
                                                .attr({'data-value': $target.parent().attr('data-value'), 'data-filters-group-filters': $target.parent().attr('data-filters-group')}); // link to anchor's parent - li tag
...
var filters = '';
var AttributeValue = [];
var GetAllFiltersFromToolBox = '';
var GetHeadAttributeName = [];

var GetFilters = [];
//var CurrentPage = window.location.pathname; // Have PRoblem When go to Second Page
var CurrentPage = defaultShopPage.url.replaceAll('&amp;', '&');

var ShowProductPerPage = '';
var showperpage = '&show=' + DefPerPage;
var DefaultProductPerPage = DefPerPage;

var showmoreclick = '';

$('.loading-more #show-more-button').on('click', function () {




    if ((parseInt($('.loading-more #show-more-button').attr('data-current-show')) + parseInt(DefaultProductPerPage)) >= parseInt($('.loading-more #show-more-button').attr('data-max'))) {
        $(this).hide();
    }

    ShowProductPerPage = parseInt($(this).attr('data-current-show')) + parseInt(DefaultProductPerPage);

    $(this).attr('data-current-show', ShowProductPerPage);


    $('html, body').animate({scrollTop: $('#BenefitTerm').offset().top - 100}, 'slow');
...
$('#filter-clean-nofound').on('click', function () {
    $('.filter-clean').trigger("click");
    //location.reload();
});
...
$('.toolbox .sorting').on('click', function () {
    $(this).removeClass('active');
    $('#get-sorting').val($(this).attr('data-sorting')).trigger('change');
    $(this).addClass('active');
});
$('.toolbox #get-sorting').on('change', function () {
    showmoreclick = 0;
    ActiveFilter();
});
...
function ActiveFilter() {
    ...
    $('.products').html('<div class="loader"><div class="loading"></div></div>');
    $('.products').css('height', '200px');


    orderby = $('.toolbox #get-sorting').val();

    //Need Timeout to get Filters from toolbox if no have timeout not get
    setTimeout(function () {

        GetAllFiltersFromToolBox = '';
        GetHeadAttributeName = [];
        GetFilters = [];

        //Get all Filters from ToolBox
        GetAllFiltersFromToolBox = $('.select-items a.filters').map(function () {
            return {name: $(this).attr("data-filters-group-filters"), value: $(this).attr('data-value')};
        }).get();

        //Make foreach with attribute name
        $.each(GetAllFiltersFromToolBox, function (i, val) {
            GetHeadAttributeName.indexOf(val.name) === -1 ? GetHeadAttributeName.push(val.name) : '';
        });

        //Get AllFilteers
        $.each(GetHeadAttributeName, function (i, val) {
            GetFilters.push({name: val, value: returnfilterval(val)});
        });
        ...
        filters = '';
        //Make Filters Path
        $.each(GetFilters, function (i, val) {
            var newquery = val.name.replace("filter_", "query_type_");
            filters += '&' + val.name + '=' + val.value;
            if (filters.indexOf(newquery) > -1) {
        ...
        if (ShowProductPerPage) {
            showperpage = '&show=' + ShowProductPerPage;
        }
        ...
        if (filters) {
            ...
            if (post_type && search) {
                var loadingUrl = CurrentPage + '&orderby=' + orderby + showperpage + filters;
            } else {
                var loadingUrl = CurrentPage + '?orderby=' + orderby + showperpage + filters;
            }
            ...
            $('.products').load(loadingUrl + ' .products', function (response, status, xhr) {
                var numItems = $('.product').length;
                var max = parseInt($('.loading-more #show-more-button').attr('data-max'));
                if (numItems < parseInt(DefaultProductPerPage)) {
                    $('.loading-more #show-more-button').hide();
                } else {

                    if ((parseInt($('.loading-more #show-more-button').attr('data-current-show')) + parseInt(DefaultProductPerPage)) >= parseInt($('.loading-more #show-more-button').attr('data-max'))) {
                        $('.loading-more #show-more-button').hide();
                    } else {
                        $('.loading-more #show-more-button').show();
                    }

                }
                $('.products').css('height', '100%');

                if (numItems === 0) {
                    $('.products').html('<div class="container-fluid"><p class="woocommerce-info woocommerce-no-products-found" style="font-size:30px;">Pro zadané filtry nebyly nalezeny žádné výsledky</p><div class="select-items" style="display: block;text-align: center;"><a id="filter-clean-nofound" href="" class="btn text-primary">Clean All</a></div></div>');
                }

                if (showmoreclick === 1) {
                    $([document.documentElement, document.body]).animate({
                        scrollTop: $("#show-more-button").offset().top - 1200
                    }, 2000);
                }
            });
            // $('.woocommerce-pagination').load(CurrentPage + '?orderby=' + orderby + showperpage + filters + ' .woocommerce-pagination');
        } else {

            if (post_type && search) {
                var loadingUrl = CurrentPage + '&orderby=' + orderby + showperpage;
            } else {
                var loadingUrl = CurrentPage + '?orderby=' + orderby + showperpage;
            }
            ...
            // $('.woocommerce-pagination').load(CurrentPage + '?orderby=' + orderby + ' .woocommerce-pagination');
        }

    }, 850);
}
function returnfilterval(val) {
    //Return Value of Filter(attribute - value)
    AttributeValue = '';
    AttributeValue = $('.select-items a.filters[data-filters-group-filters="' + val + '"]').map(function () {
        //AttributeValue.push({key: val, value: $(this).attr('data-value')});
